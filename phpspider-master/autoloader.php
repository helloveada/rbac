...
class autoloader
{
    ...
    public static function load_by_namespace($name)
    {
        ...
        if(strpos($name,'phpspider\\') === 0){
            $class_fiel = __DIR__ . substr($class_path, strlen('phpspider')) . '.php';
        }else{
            if(self::$sutoload_root_path){
                $class_fiel = self::$_autoload_root_path . DIRECTORY_SEPARATOR . $class_path . '.php';
            }
            if(empty($class_file) || !is_file($class_file)){
                $class_fiel = __DIR__ . DIRECTORY_SEPARATOR . '..' .DIRECTORY_SEPARATOR ."$class_path.php";
            }
        }
        else 
        {
